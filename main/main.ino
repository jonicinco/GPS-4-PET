...
bool trySend() {
    ...
    if (0 < gps_hdop() && gps_hdop() < 50 && gps_latitude() != 0 && gps_longitude() != 0 && gps_altitude() != 0) {
        ...
        //delay(1500);        //Nos da tiempo de leer los datos de localización... 
    ...
    ttn_send(txBuffer, sizeof(txBuffer), LORAWAN_PORT, false);
        ...
        // Mensaje de éxito al enviar paquetes
        displayMessage("Paquete enviado con éxito");
        return true;
    } else {
        displayMessage("Esperando datos GPS válidos...");
        return false;
    }
}
...
void callback(uint8_t message) {
    ...
    if (EV_JOINING == message) {
       ...
       // delay(1500);        //Nos da tiempo de leer los mensajes por la pantalla OLED.
    }
...
// Mostramos mensajes por la pantalla OLED
    switch (message) {
        case EV_JOINING:
            displayMessage("Conectando a TTN...");
            break;
        case EV_JOINED:
            displayMessage("Conexión establecida con TTN");
            break;
        case EV_TXCOMPLETE:
            displayMessage("Paquete transmitido con éxito");
            break;
        case EV_TXCANCELED:
        displayMessage("Fallo al enviar paquete");
        break;
    ...
    if (EV_TXCOMPLETE == message && packetQueued) {
        ...
        packetQueued = false;
        packetSent = true;
        delay(1000);        //tiempo de lectura de mensajes. 
    }
